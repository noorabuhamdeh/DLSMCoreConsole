...
namespace DLSMConsoleCore
{
    ...
    public class DLMSReader : IDLMSReader
    {
        ...
        public DLMSReader(ILogger _logger, GXDLMSSecureClient client, IGXMedia media, string invocationCounter = null)
        {
            logger = _logger;
            Media = media;
            Client = client;
        ...
        public void SNRMRequest()
        {
            ...
            if (data != null)
            {
                logger.Debug("Send SNRM request." + GXCommon.ToHex(data, true));
                ReadDataBlock(data, reply);
                logger.Debug("Parsing UA reply." + reply.ToString());

                //Has server accepted client.
                Client.ParseUAResponse(reply.Data);
                logger.Debug("Parsing UA reply succeeded.");
            }
        }
        ...
        public void AarqRequest()
        {
            ...
            {
                logger.Debug("Send AARQ request", GXCommon.ToHex(it, true));
                reply.Clear();
                ReadDataBlock(it, reply);
            }

            logger.Debug("Parsing AARE reply" + reply.ToString());
            //Parse reply.
            ...
            logger.Debug("Parsing AARE reply succeeded.");
        }
            ...
            if (!string.IsNullOrEmpty(InvocationCounter) && Client.Ciphering != null && Client.Ciphering.Security != (byte)Security.None)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        logger.Debug("Invocation counter: " + Convert.ToString(Client.Ciphering.InvocationCounter));
                        reply.Clear();
                        Disconnect();
                    }
                    catch (Exception Ex)
        ...
        void InitializeOpticalHead()
        {
            ...
            logger.Debug("IEC received: " + p.Reply);
            int pos = 0;
            ...
            logger.Debug("BaudRate is : " + BaudRate.ToString());
            //Send ACK
            ...
            logger.Debug("Moving to mode E.", arr);
        ...
        {
            logger.Debug("Standard: " + Client.Standard);
            if (Client.Ciphering.Security != (byte)Security.None)
            {
                logger.Debug("Security: " + Client.Ciphering.Security);
                logger.Debug("System title: " + GXCommon.ToHex(Client.Ciphering.SystemTitle, true));
                logger.Debug("Authentication key: " + GXCommon.ToHex(Client.Ciphering.AuthenticationKey, true));
                logger.Debug("Block cipher key " + GXCommon.ToHex(Client.Ciphering.BlockCipherKey, true));
                if (Client.Ciphering.DedicatedKey != null)
                {
                    logger.Debug("Dedicated key: " + GXCommon.ToHex(Client.Ciphering.DedicatedKey, true));
                }
            }
            UpdateFrameCounter();
            InitializeOpticalHead();
        ...
        public void GetScalersAndUnits()
        {
            ...
            //If trace is info.
            logger.Debug("Read scalers and units from the device.");
        ...
        public void ShowValue(object val, int pos)
        {
            //If trace is info.
                ...
                logger.Debug("Index: " + pos + " Value: " + val);
        }
        ...
        public void GetProfileGenerics()
        {
            ...
            {
                //If trace is info.
                logger.Debug("-------- Reading " + it.GetType().Name + " " + it.Name + " " + it.Description);
                long entriesInUse = -1;
                ...
                //If trace is info.
                logger.Debug("Entries: " + entriesInUse + "/" + entries);
                ...
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        logger.Error("Error! Failed to read first row: " + ex.Message);
                        //Continue reading.
                    }
                }
                //All meters are not supporting parameterized read.
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        logger.Error("Error! Failed to read last day: " + ex.Message);
                        //Continue reading.
                    }
                }
            }
        ...
        public void GetReadOut()
        {
            foreach (GXDLMSObject it in Client.Objects)
            {
                ...
                if (!(it is IGXDLMSBase))
                {
                    ...
                    logger.Debug("Unknown Interface: " + it.ObjectType.ToString());
                    continue;
                }
                logger.Debug("-------- Reading " + it.GetType().Name + " " + it.Name + " " + it.Description);
                foreach (int pos in (it as IGXDLMSBase).GetAttributeIndexToRead(true))
                {
                    ...
                    {
                        logger.Error("Error! " + it.GetType().Name + " " + it.Name + "Index: " + pos + " " + ex.Message);
        ...
        public void ReadDLMSPacket(byte[] data, GXReplyData reply)
        {
            ...
            lock (Media.Synchronous)
            {
                while (!succeeded && pos != 3)
                {
                    ...
                    {
                        logger.Debug("TX:\t" + DateTime.Now.ToLongTimeString() + "\t" + GXCommon.ToHex(data, true));
                        p.Reply = null;
                        Media.Send(data, null);
                    }
                    ...
                    if (!succeeded)
                    {
                        ...
                        //Try to read again...
                        logger.Debug("Data send failed. Try to resend " + pos.ToString() + "/3");
                    }
                }
                ...
                {
                    logger.Error("RX:\t" + DateTime.Now.ToLongTimeString() + "\t" + rd);
                    throw ex;
                }
            }
            logger.Debug("RX:\t" + DateTime.Now.ToLongTimeString() + "\t" + rd);
            if (reply.Error != 0)
            {
        ...
        public void Disconnect()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    logger.Debug("Disconnecting from the meter.");
                    GXReplyData reply = new GXReplyData();
        ...
        public void Release()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    logger.Debug("Release from the meter.");
                    GXReplyData reply = new GXReplyData();
                ...
                catch (Exception ex)
                {
                    ...
                    logger.Error("Release failed. " + ex.Message);
                }
            }
        }
