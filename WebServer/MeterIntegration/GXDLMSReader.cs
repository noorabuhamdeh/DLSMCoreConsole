...
{
    public interface IDLMSReader
    {
        int Id { get; }
        string Name { get; }
        GXDLMSSecureClient InnerClient { get; }
        string GetAssociationViewsXml();
        void LoadAssociationViewsFromXml(string xml);
        void AarqRequest();
        void Close();
        void Disconnect();
        bool OpenMedia();
        bool GetAssociationView(string outputFile);
        void GetProfileGenericColumns();
        void GetProfileGenerics();
        void GetReadOut();
        void GetScalersAndUnits();
        void ImageUpdate(GXDLMSImageTransfer target, byte[] identification, byte[] data);
        void InitializeConnection();
        void Method(GXDLMSObject it, int attributeIndex, object value, DataType type);
        object Read(GXDLMSObject it, int attributeIndex);
        void ReadAll(string outputFile);
        void ReadDataBlock(byte[] data, GXReplyData reply);
        bool ReadDataBlock(byte[][] data, GXReplyData reply);
        void ReadDLMSPacket(byte[] data, GXReplyData reply);
        void ReadList(List<KeyValuePair<GXDLMSObject, int>> list);
        object[] ReadRowsByEntry(GXDLMSProfileGeneric it, uint index, uint count);
        object[] ReadRowsByRange(GXDLMSProfileGeneric it, DateTime start, DateTime end);
        void Release();
        void ShowValue(object val, int pos);
        void SNRMRequest();
        void Write(GXDLMSObject it, int attributeIndex);
        void WriteList(List<KeyValuePair<GXDLMSObject, int>> list);
    }

    public class DLMSReader : IDLMSReader
    {
        /// <summary>
        /// Wait time in ms.
        /// </summary>
        ...
        ILogger<DLMSReader> logger = null;
        string name;
        int id;

        public string Name => name;
        public int Id => id;
        public GXDLMSSecureClient InnerClient => Client;
        /// <summary>
        /// Constructor.
        /// </summary>
        ...
        public DLMSReader(int _id, string _name, GXDLMSSecureClient client, IGXMedia media, string invocationCounter = null)
        {
            logger = new Serilog.Extensions.Logging.SerilogLoggerFactory().CreateLogger<DLMSReader>();
            id = _id;
            name = _name;
            Media = media;
            Client = client;
        ...
        public void SNRMRequest()
        {
            ...
            if (data != null)
            {
                logger.LogDebug("Send SNRM request." + GXCommon.ToHex(data, true));
                ReadDataBlock(data, reply);
                logger.LogDebug("Parsing UA reply." + reply.ToString());

                //Has server accepted client.
                Client.ParseUAResponse(reply.Data);
                logger.LogDebug("Parsing UA reply succeeded.");
            }
        }
        ...
        public void AarqRequest()
        {
            ...
            {
                logger.LogDebug("Send AARQ request", GXCommon.ToHex(it, true));
                reply.Clear();
                ReadDataBlock(it, reply);
            }

            logger.LogDebug("Parsing AARE reply" + reply.ToString());
            //Parse reply.
            ...
            logger.LogDebug("Parsing AARE reply succeeded.");
        }
            ...
            if (!string.IsNullOrEmpty(InvocationCounter) && Client.Ciphering != null && Client.Ciphering.Security != (byte)Security.None)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        logger.LogDebug("Invocation counter: " + Convert.ToString(Client.Ciphering.InvocationCounter));
                        reply.Clear();
                        Disconnect();
                    }
                    catch (Exception Ex)
        ...
        void InitializeOpticalHead()
        {
            ...
            string data = "/?!\r\n";
            logger.LogDebug("IEC Sending:" + data);
            ...
            logger.LogDebug("IEC received: " + p.Reply);
            int pos = 0;
            ...
            logger.LogDebug("BaudRate is : " + BaudRate.ToString());
            //Send ACK
            ...
            logger.LogDebug("Moving to mode E.", arr);
            ...
            lock (Media.Synchronous)
            {
                ...
                if (p.Reply != null)
                {
                    logger.LogDebug("Received: " + p.Reply);
                }
        ...
        {
            logger.LogDebug("Standard: " + Client.Standard);
            if (Client.Ciphering.Security != (byte)Security.None)
            {
                logger.LogDebug("Security: " + Client.Ciphering.Security);
                logger.LogDebug("System title: " + GXCommon.ToHex(Client.Ciphering.SystemTitle, true));
                logger.LogDebug("Authentication key: " + GXCommon.ToHex(Client.Ciphering.AuthenticationKey, true));
                logger.LogDebug("Block cipher key " + GXCommon.ToHex(Client.Ciphering.BlockCipherKey, true));
                if (Client.Ciphering.DedicatedKey != null)
                {
                    logger.LogDebug("Dedicated key: " + GXCommon.ToHex(Client.Ciphering.DedicatedKey, true));
                }
            }
            UpdateFrameCounter();
            InitializeOpticalHead();
        ...
        public void GetScalersAndUnits()
        {
            ...
            //If trace is info.
            logger.LogDebug("Read scalers and units from the device.");
            ...
            if ((Client.NegotiatedConformance & Gurux.DLMS.Enums.Conformance.MultipleReferences) == 0)
            {
                ...
                foreach (GXDLMSObject it in objs)
                {
                    try
                    {
                        ...
                        {
                            logger.LogDebug(it.Name.ToString());
                            Read(it, 3);
                        }
                        if (it is GXDLMSDemandRegister && (it.GetAccess(4) & AccessMode.Read) != 0)
                        {
        ...
        public void GetProfileGenericColumns()
        {
            ...
            foreach (GXDLMSObject it in Client.Objects.GetObjects(ObjectType.ProfileGeneric))
            {
                try
                {
                    logger.LogDebug(it.LogicalName);
                    Read(it, 3);
                    //If info.
        ...
        public void ShowValue(object val, int pos)
        {
            //If trace is info.
                ...
                logger.LogDebug("Index: " + pos + " Value: " + val);
        }
        ...
        public void GetProfileGenerics()
        {
            ...
            {
                //If trace is info.
                logger.LogDebug("-------- Reading " + it.GetType().Name + " " + it.Name + " " + it.Description);
                long entriesInUse = -1;
                ...
                //If trace is info.
                logger.LogDebug("Entries: " + entriesInUse + "/" + entries);
                ...
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        logger.LogError("Error! Failed to read first row: " + ex.Message);
                        //Continue reading.
                    }
                }
                //All meters are not supporting parameterized read.
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        logger.LogError("Error! Failed to read last day: " + ex.Message);
                        //Continue reading.
                    }
                }
            }
        ...
        public void GetReadOut()
        {
            foreach (GXDLMSObject it in Client.Objects)
            {
                ...
                if (!(it is IGXDLMSBase))
                {
                    ...
                    logger.LogDebug("Unknown Interface: " + it.ObjectType.ToString());
                    continue;
                }
                logger.LogDebug("-------- Reading " + it.GetType().Name + " " + it.Name + " " + it.Description);
                foreach (int pos in (it as IGXDLMSBase).GetAttributeIndexToRead(true))
                {
                    ...
                    {
                        logger.LogError("Error! " + it.GetType().Name + " " + it.Name + "Index: " + pos + " " + ex.Message);
        ...
        public void ReadDLMSPacket(byte[] data, GXReplyData reply)
        {
            ...
            lock (Media.Synchronous)
            {
                while (!succeeded && pos != 3)
                {
                    ...
                    {
                        logger.LogDebug("TX:\t" + DateTime.Now.ToLongTimeString() + "\t" + GXCommon.ToHex(data, true));
                        p.Reply = null;
                        Media.Send(data, null);
                    }
                ...
                {
                    logger.LogError("RX:\t" + DateTime.Now.ToLongTimeString() + "\t" + rd);
                    throw ex;
                }
            }
            logger.LogDebug("RX:\t" + DateTime.Now.ToLongTimeString() + "\t" + rd);
            if (reply.Error != 0)
            {
        ...
        public void Disconnect()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    logger.LogDebug("Disconnecting from the meter.");
                    GXReplyData reply = new GXReplyData();
        ...
        public void Release()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    logger.LogDebug("Release from the meter.");
                    GXReplyData reply = new GXReplyData();
                ...
                catch (Exception ex)
                {
                    ...
                    logger.LogError("Release failed. " + ex.Message);
                }
            }
        }
        ...
        public string GetAssociationViewsXml()
        {
            try
            {
                OpenMedia();
                InitializeConnection();
                GetAssociationView(null);
                
                using (var memoryStream = new MemoryStream())
                {
                    Client.Objects.Save(memoryStream, new GXXmlWriterSettings() { UseMeterTime = true, IgnoreDefaultValues = false });
        ...
        public void LoadAssociationViewsFromXml(string xml)
        {
